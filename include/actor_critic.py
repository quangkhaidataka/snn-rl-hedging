...
from include.network import Actor, ActorSNN,CriticSNN
...
class ActorCritic(object):
    def __init__(self, state_dim, s = getSettings()):
        ...
        # ---- actor: ANN vs SNN (select from settings) ----
        actor_type = str(s.get('actor_type', 'ANN')).upper()
        # actor_type = s['actor_type']
        print(f'Current actor type is {actor_type}')
        if actor_type == 'SNN':
            ActorClass = ActorSNN
            CriticClass = Critic
        else:
        ...
        self.actor = ActorClass(state_dim, s).to(device)
        self.actor_target = ActorClass(state_dim, s).to(device)
        self.actor_optimizer = torch.optim.Adam(self.actor.parameters(), lr=s['actor_lr'])

        # Initialize two critics and their targets (e.g., for TD3 or similar algorithms)
        self.critic1 = CriticClass(state_dim, 1, s).to(device)
        self.critic_target1 = CriticClass(state_dim, 1, s).to(device)
        self.critic_opt1 = torch.optim.Adam(self.critic1.parameters(), lr=s['critic_lr'])
        
        self.critic2 = CriticClass(state_dim, 1, s).to(device)
        self.critic_target2 = CriticClass(state_dim, 1, s).to(device)
    ...
    def act(self, state):
        with torch.no_grad():
            self.actor.eval()
            a = self.actor(torch.as_tensor(state, dtype=torch.float32, device=device))
